...
export default function Home() {
  ...
  // Community (MyAnimeList forum threads for top airing anime, via Jikan, server-side —
  // Jikan doesn't send CORS headers for browser use, so this stays a server route)
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <Panel className="fade-in-up" style={{ padding: "10px 14px", background: COLORS.ink }}>
              <p style={{ fontSize: 12, color: COLORS.muted, margin: 0 }}>
                Live MyAnimeList forum discussions for the top currently-airing anime, fetched server-side.
              </p>
            </Panel>
            {threadsLoading && (
            ...
            {threadsError && (
              <Panel className="fade-in-up" style={{ padding: 16 }}>
                <p style={{ margin: 0, fontSize: 13, color: "#5A5648" }}>Couldn't reach MyAnimeList right now: {threadsError}</p>
              </Panel>
            )}
            {threads.map((t, i) => (
                ...
                <Panel className="fade-in-up hover-lift" style={{ padding: "12px 14px", display: "flex", gap: 12, alignItems: "center", animationDelay: `${i * 30}ms` }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ marginBottom: 2 }}>
                      <Badge color={COLORS.muted}>{t.animeTitle}</Badge>
                    </div>
                    <p style={{ margin: 0, fontWeight: 600, fontSize: 14 }}>{t.title}</p>
                    <p style={{ margin: "2px 0 0", fontSize: 12, color: COLORS.muted }}>
                      by {t.author || "unknown"} · {t.replies} replies · {t.date ? timeAgo(t.date) : ""}
                    </p>
                  </div>
